...
void Server::process_packet(const int id, const int packet_size, const char * buf)
{
	...
	case CS_CHARACTER_SELECT:
	{
		...
		db_set_character(id);
		send_init_packet(id);//몬스터, 아이템, 기타 정보들을 나에게
		send_my_status_to_all_packet(id);//내정보를 다른 모두에게
	}
	break;
	case CS_INFO:
	{
...
void Server::send_init_packet(int id)	// 아이템, 몬스터, 유저 데이터를 다른 접속이 없으면 SC_ID로, 있으면 SC_INIT_DATA로 보냄
{
	int kind = -1;
	flatbuffers::FlatBufferBuilder builder;
	builder.Clear();
